...
import { Mail, Trash2, CheckCheck, Lock, Eye, EyeOff, RefreshCw } from "lucide-react";
...
export default function AdminPage() {
  ...
  const fetchMessages = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/messages?key=${key}`);
  ...
  const markAsRead = async (id: string) => {
    await fetch(`/api/admin/messages?key=${key}`, {
      method: "PATCH",
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-20">
        ...
        >
          ...
          <p className="text-xs text-dark-400 mb-4">Enter your admin key to view messages</p>
          <div className="flex items-center gap-2 mb-3">
            ...
            <button
              ...
              className="p-2 text-dark-400 hover:text-dark-700"
            >
          ...
          <button
            ...
            className="w-full px-4 py-2 bg-primary-500 text-white text-xs font-bold rounded-lg hover:bg-primary-600 disabled:opacity-50"
          >
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-xs text-dark-400">
                <span className="font-bold text-primary-500">{stats.unread}</span> unread
              </p>
              <p className="text-xs text-dark-400">
            ...
            <button
              onClick={fetchMessages}
              className="p-2 text-dark-400 hover:text-dark-700 rounded-lg hover:bg-dark-50"
            >
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {!msg.read && (
                      <span className="text-[8px] font-bold text-primary-500 bg-primary-100 px-1.5 py-0.5 rounded uppercase tracking-wider">
                        New
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1">
                  {!msg.read && (
                    <button
                      ...
                      className="p-1.5 text-dark-400 hover:text-primary-500 rounded"
                      title="Mark as read"
                    >
